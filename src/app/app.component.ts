...
export class AppComponent {
  ...
  constructor(private sanitizer: DomSanitizer){
    this.getVideos(0, this.limit);    
  } 
  /**
   * Get data whenever scroll down
   */
  onScrollDown() {
    this.start += this.limit;
    this.getVideos(this.start, this.start + this.limit);    
  }
   ...
   * @param end end item
   */
  getVideos(start=0, end=5) {
    let videosNext = this._ytData.slice(start,end);
    if(videosNext.length) {
      this.videos = this.videos.concat(videosNext);
    }
  }
